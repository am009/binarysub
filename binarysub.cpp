...
UTypePtr make_utop() { return std::make_shared<UType>(UTop{}); }

UTypePtr make_ubot() { return std::make_shared<UType>(UBot{}); }
      ...
      [&](auto const &n) -> UTypePtr {
        ...
        if constexpr (isTPrimitiveType<T>()) {
          ...
          for (auto const &[name, ty] : n.fields) {
            fields.emplace_back(
                name, coalesceTypeImpl(ty, polar, inProcess, recursive));
          }
          return make_urecordtype(std::move(fields));
        } else if constexpr (isTVariableType<T>()) {
          PolarVar pv{n.state.get(), polar};

          // Check if we're already processing this variable (recursion
          // detection)
          ...
          const auto &bounds =
              polar ? n.state->lowerBounds : n.state->upperBounds;
          ...
          for (const auto &bound : bounds) {
            boundTypes.push_back(
                coalesceTypeImpl(bound, polar, inProcess, recursive));
          }

          // Combine bounds with union (for positive) or intersection (for
          // negative)
          UTypePtr result;
          if (polar) {
          ...
          static_assert(!sizeof(T),
                        "Unhandled variant type in coalesceTypeImpl");
        }
      },
      st->v);
}
  ...
  std::visit(
      [&](auto const &n) {
        ...
        if constexpr (std::is_same_v<T, UTop>) {
          ...
          oss << "FUN(" << computeTypeHash(n.lhs) << "->"
              << computeTypeHash(n.rhs) << ")";
        } else if constexpr (std::is_same_v<T, UUnion>) {
          oss << "UNION(" << computeTypeHash(n.lhs) << "+"
              << computeTypeHash(n.rhs) << ")";
        } else if constexpr (std::is_same_v<T, UInter>) {
          oss << "INTER(" << computeTypeHash(n.lhs) << "&"
              << computeTypeHash(n.rhs) << ")";
        } else if constexpr (std::is_same_v<T, URecordType>) {
          oss << "REC{";
          for (size_t i = 0; i < n.fields.size(); ++i) {
            if (i > 0)
              oss << ";";
            oss << n.fields[i].first << ":"
                << computeTypeHash(n.fields[i].second);
          }
          oss << "}";
        } else if constexpr (std::is_same_v<T, URecursiveType>) {
  ...
  std::visit(
      [&](auto const &n) {
        ...
        if constexpr (std::is_same_v<T, UTop>) {
          ...
          static_assert(!sizeof(T),
                        "Unhandled UType variant in analyzeOccurrencesImpl");
        }
      },
      ty->v);
}
  ...
  return std::visit(
      [&](auto const &n) -> UTypePtr {
        ...
        if constexpr (std::is_same_v<T, UTop>) {
          ...
          return make_ufunctiontype(removePolarVariables(n.lhs, occMap),
                                    removePolarVariables(n.rhs, occMap));
        } else if constexpr (std::is_same_v<T, UUnion>) {
          auto left = removePolarVariables(n.lhs, occMap);
          auto right = removePolarVariables(n.rhs, occMap);
          // Simplify: ⊥ ∪ τ = τ
          if (std::holds_alternative<UBot>(left->v))
            return right;
          if (std::holds_alternative<UBot>(right->v))
            return left;
          ...
          auto right = removePolarVariables(n.rhs, occMap);
          // Simplify: ⊤ ∩ τ = τ
          if (std::holds_alternative<UTop>(left->v))
            return right;
          if (std::holds_alternative<UTop>(right->v))
            return left;
          ...
          return make_urecursivetype(n.name,
                                     removePolarVariables(n.body, occMap));
        } else {
          static_assert(!sizeof(T),
                        "Unhandled UType variant in removePolarVariables");
        }
      },
      ty->v);
}
      ...
      [&](auto const &n) -> UTypePtr {
        ...
        if constexpr (std::is_same_v<T, UTop>) {
          ...
          if (varId != 0) {
            ...
            if (it != occMap.end()) {
              const auto &occ = it->second;
              // If variable co-occurs with exactly one primitive both
              // positively and negatively, it's equivalent to that primitive
              // (variable sandwich)
              if (occ.appearsPositive && occ.appearsNegative &&
                  occ.coOccurs.positivePrims.size() == 1 &&
                  occ.coOccurs.negativePrims.size() == 1 &&
                  *occ.coOccurs.positivePrims.begin() ==
                      *occ.coOccurs.negativePrims.begin()) {
          ...
          return make_ufunctiontype(flattenVariableSandwiches(n.lhs, occMap),
                                    flattenVariableSandwiches(n.rhs, occMap));
        } else if constexpr (std::is_same_v<T, UUnion>) {
          return make_uunion(flattenVariableSandwiches(n.lhs, occMap),
                             flattenVariableSandwiches(n.rhs, occMap));
        } else if constexpr (std::is_same_v<T, UInter>) {
          return make_uinter(flattenVariableSandwiches(n.lhs, occMap),
                             flattenVariableSandwiches(n.rhs, occMap));
        } else if constexpr (std::is_same_v<T, URecordType>) {
          std::vector<std::pair<std::string, UTypePtr>> fields;
          fields.reserve(n.fields.size());
          for (const auto &[name, fieldType] : n.fields) {
            fields.emplace_back(name,
                                flattenVariableSandwiches(fieldType, occMap));
          }
          return make_urecordtype(std::move(fields));
        } else if constexpr (std::is_same_v<T, URecursiveType>) {
          return make_urecursivetype(n.name,
                                     flattenVariableSandwiches(n.body, occMap));
        } else {
          static_assert(!sizeof(T),
                        "Unhandled UType variant in flattenVariableSandwiches");
        }
      },
      ty->v);
}
...
UTypePtr unifyIndistinguishableVariables(const UTypePtr &ty,
  ...
  std::map<std::uint32_t, std::uint32_t>
      representatives; // var -> representative
  ...
  for (const auto &[varId, occ] : occMap) {
    for (auto otherVarId : occ.coOccurs.positiveVars) {
      ...
      if (otherIt != occMap.end() &&
        ...
        std::uint32_t rep =
            std::min(representatives[varId], representatives[otherVarId]);
  ...
  std::function<UTypePtr(const UTypePtr &)> substitute =
  ...
  UTypePtr result = std::visit(
      [&](auto const &n) -> UTypePtr {
        ...
        if constexpr (std::is_same_v<T, UTop>) {
          ...
          return make_ufunctiontype(hashConsType(n.lhs, hashMap),
                                    hashConsType(n.rhs, hashMap));
        } else if constexpr (std::is_same_v<T, UUnion>) {
          return make_uunion(hashConsType(n.lhs, hashMap),
                             hashConsType(n.rhs, hashMap));
        } else if constexpr (std::is_same_v<T, UInter>) {
          return make_uinter(hashConsType(n.lhs, hashMap),
                             hashConsType(n.rhs, hashMap));
        } else if constexpr (std::is_same_v<T, URecordType>) {
          std::vector<std::pair<std::string, UTypePtr>> fields;
          fields.reserve(n.fields.size());
...
CompactTypeScheme compactType(const SimpleType &st) {
  std::map<std::pair<VariableState *, bool>, std::uint32_t>
      recursive_vars; // (var, polarity) -> recursive_id
  ...
  std::function<std::shared_ptr<CompactType>(const SimpleType &, bool,
                                             std::set<VariableState *> &)>
      go;

  go = [&](const SimpleType &ty, bool pol,
           std::set<VariableState *> &parents) -> std::shared_ptr<CompactType> {
    return std::visit(
        [&](auto const &n) -> std::shared_ptr<CompactType> {
          ...
          if constexpr (std::is_same_v<T, TPrimitive>) {
            ...
            auto ct = make_empty_compact_type();
            ct->function =
                std::make_pair(go(n.lhs, !pol, parents), // contravariant
            ...
            auto ct = make_empty_compact_type();
            if (!n.fields.empty()) {
              auto rec_map = std::make_shared<
                  std::map<std::string, std::shared_ptr<CompactType>>>();
            ...
            const auto &bounds =
                pol ? n.state->lowerBounds : n.state->upperBounds;
            ...
            static_assert(!sizeof(T),
                          "Unhandled SimpleType variant in compactType");
          }
        },
        ty->v);
  };
...
UTypePtr simplifyType(const UTypePtr &ty) {
  ...
  // Remove variables that only occur in one polarity (similar to compact's
  // polar analysis)
  ...
  // Step 4: Unify indistinguishable variables (similar to compact's
